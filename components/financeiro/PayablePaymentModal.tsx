...
import { ContaPagar, Pagamento } from "@/lib/storage";
...
export function PayablePaymentModal({ conta, onClose, onSuccess }: PayablePaymentModalProps) {
    ...
    return (
        ...
        >
            <div className="mb-6 p-4 bg-surface-secondary border border-border rounded-xl space-y-1">
                <p className="text-sm text-text-secondary">Conta: <span className="font-medium text-text-primary">#{conta.numeroNF || conta.id.slice(0, 8)} - {conta.fornecedor.nome}</span></p>
                ...
                <p className="text-sm text-text-secondary">Saldo Restante: <span className="font-bold text-red-600">R$ {conta.saldoRestante.toLocaleString('pt-BR', { minimumFractionDigits: 2 })}</span></p>
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div className="flex items-center justify-between pt-2">
                    <span className="text-sm text-text-secondary">Lançar despesa no Fluxo de Caixa</span>
                    <Toggle
